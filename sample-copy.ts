...
} from '../automation/human-behavior.js'

//mớiii

// Pure helper function to normalize input data
const buildNormalizedInput = (input: any) => {
    ...
    const baseItems = Array.isArray((input as any).items) && (input as any).items.length > 0 ? (input as any).items as any[] : [input as any]
...
export async function run(page: Page, input: any = {}) {
