  ...
  const normalizeRecord = (raw: Record<string, any> = {}): NormalizedInput => {
    try {
      ...
      return {
        ...
        recentTabComment: recentTabComment
          ? parseInt(recentTabComment)
          : undefined,
 ...
 */
const parseShopeeLinks = (
  linkShopee: string | undefined
): string[] | undefined => {
...
const handleComment = async (
    ...
    if (normalizedInput.commentPool && normalizedInput.commentPool.length > 0) {
      commentText =
        normalizedInput.commentPool[
          Math.floor(Math.random() * normalizedInput.commentPool.length)
        ];
  ...
  if (normalizedInput.linkShopee && normalizedInput.linkShopee.length > 0) {
    const randomLink =
      normalizedInput.linkShopee[
        Math.floor(Math.random() * normalizedInput.linkShopee.length)
      ];
...
const CommentFeedsAndSearch = async (
  ...
  try {
    ...
    if (mode === "topTabComment" && normalizedInput.topTabComment) {
      ...
      mode === "recentTabComment" &&
      normalizedInput.recentTabComment
    ) {
    ...
    console.log(
      `[CommentSearchKeyWord] Mode: ${mode}, Comment count: ${commentCount}`
    );
    ...
    let scope = await page
      .waitForSelector(SCOPE_SELECTOR, { timeout: 15000 })
      .catch(() => null);
    if (!scope) {
      scope = await page
        .waitForSelector(FALLBACK_SCOPE_SELECTOR, { timeout: 10000 })
        .catch(() => null);
      if (!scope) {
        throw new Error(
          `Scope not found for selectors: ${SCOPE_SELECTOR} | ${FALLBACK_SCOPE_SELECTOR}`
        );
      }
    }
    ...
    for (
      let commentIteration = 0;
      commentIteration < commentCount;
      commentIteration++
    ) {
      console.log(
        `[CommentSearchKeyWord] Starting comment iteration ${
          commentIteration + 1
        }/${commentCount}`
      );
      ...
      const updatedPosts = await scope.$$(
        'div[data-pressable-container="true"]'
      );
      if (updatedPosts.length > posts.length) {
        console.log(
          `[feed] Lazy loading triggered: ${
      ...
      const replyIconSelector =
        'div[role="button"] svg[aria-label="Reply"], div[role="button"] svg[aria-label="Trả lời"]';
      ...
      if (replyIcon) {
        ...
        try {
          ...
          console.log(
            "[feed] Cursor moved to safe area using multiple methods"
          );
        } catch (e) {
          console.log(
            "[feed] Could not move cursor:",
            e instanceof Error ? e.message : String(e)
          );
        }
        ...
        try {
          ...
          if (composer) {
            ...
            try {
              ...
              if (statusElement) {
                const rawText = await statusElement.evaluate(
                ...
                statusText = rawText
                  ...
                  .trim();
                console.log(
                  `[feed] Status text: "${statusText.slice(0, 100)}..." (len=${
                    statusText.length
                  })`
                );
                if (!statusText) {
                  console.log(
                    "[feed] Warning: statusText is empty after cleaning"
                  );
                }
              } else {
                console.log(
                  "[feed] Status text element not found with fallback selectors"
                );
              }
            } catch (e) {
              console.log(
                "[feed] Error getting status text:",
                e instanceof Error ? e.message : String(e)
              );
            }

            // Use handleComment function to generate comment
            const commentText = await handleComment(
              normalizedInput,
              statusText
            );
        ...
        await humanDelay(20000, 60000);
      } else {
        console.log(
          `[feed] Reply button not found for post index=${commentIteration}, skipping`
        );
      }

      console.log(
        `Comment iteration ${commentIteration + 1}/${commentCount} completed.`
      );

      // Removed short inter-iteration delay; reading delay above simulates dwell time
    }

    console.log(
      `✅ All comment iterations completed. Total: ${commentCount} iteration(s)`
    );
  } catch (error) {
...
export async function run(page: Page, input: Input = {}) {
  ...
  try {
    ...
    console.log(
      "[input] commentPool count:",
      normalizedInput.commentPool?.length || 0
    );
    ...
    const navigateWithRetry = async (
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
          ...
          console.warn(
            `[spam.nav] attempt ${attempt}/${maxAttempts} failed. Retrying in ${backoffMs}ms...`
          );
    ...
    if (normalizedInput.feedsComment && normalizedInput.feedsComment > 0) {
      console.log(
        "Step 1b: Using comment pool for feeds comments (before search)"
      );
      await CommentFeedsAndSearch(
        page,
        normalizedInput,
        "feedsComment",
        "div.x1l7klhg"
    ...
    try {
      ...
      await humanTypeWithMistakes(
        page,
        searchInput,
        normalizedInput.searchKeyword || "nails"
      );
    ...
    if (
      ...
      normalizedInput.topTabComment &&
      normalizedInput.topTabComment > 0
    ) {
    ...
    if (
      normalizedInput.searchKeyword &&
      normalizedInput.recentTabComment &&
      normalizedInput.recentTabComment > 0
    ) {
      console.log("Step 4: Using comment pool for recent tab comments");
      // Click recent tab first
      await humanClick(
        page,
        ".x78zum5:nth-child(1) > .x6s0dn4 > .x1iyjqo2:nth-child(2) > .x1i10hfl"
      );
