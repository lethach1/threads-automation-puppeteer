...
async function run(page, input = {}) {
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const isValidFile = typeof item.mediaPath === "string" && item.mediaPath.trim().length > 0 && existsSync(item.mediaPath) && (() => {
        try {
          return statSync(item.mediaPath).isFile();
        } catch {
          return false;
        }
      })();
      if (isValidFile) {
        try {
          if (isFirst) console.log("📍 Step 4: Uploading image...");
          const addBtn = await page.$(".x1uvtmcs > .__fb-dark-mode");
          if (addBtn) {
            ...
            if (fileInput) {
              console.log("📷 Uploading image:", item.mediaPath);
              await fileInput.uploadFile(item.mediaPath);
              await humanDelay(2e3, 4e3);
            }
          } else {
            console.log("🟨 Image button not found, skipping upload");
          }
        } catch (e) {
          console.log("🟨 Image upload skipped due to error:", (e == null ? void 0 : e.message) || e);
        }
      } else if (item.mediaPath) {
        console.log("🟨 Invalid image path, skipping upload:", item.mediaPath);
      }
      if (item.tag) {
