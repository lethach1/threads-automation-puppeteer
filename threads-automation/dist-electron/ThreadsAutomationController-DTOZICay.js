  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (isFirst) {
        ...
        const pinSelectors = [
          ".xqcrz7y .xkqq1k2 .x1lliihq",
          '[data-testid="pin-button"]',
          'button[aria-label*="pin"]',
          'button[aria-label*="Pin"]',
          '.x1lliihq[role="button"]'
        ];
        let pinButton = null;
        for (const selector of pinSelectors) {
          try {
            pinButton = await page.waitForSelector(selector, { timeout: 5e3 });
            if (pinButton) {
        ...
        if (pinButton) {
          await humanDelay(1e3, 2e3);
          await humanClick(page, pinButton);
          try {
            await page.waitForSelector(".x1i10hfl:nth-child(2) > .x6s0dn4", { timeout: 5e3 });
            await humanDelay(1e3, 2e3);
            await humanClick(page, ".x1i10hfl:nth-child(2) > .x6s0dn4");
            console.log("✅ Step 15 completed: Comment pinned");
          } catch (e) {
            console.log("🟨 Confirm button not found after pin, may already be pinned");
          }
        } else {
          console.log("🟨 Pin button not found, skipping pin step");
        }
      }
