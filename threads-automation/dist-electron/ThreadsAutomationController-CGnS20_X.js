...
async function run(page, input = {}) {
  ...
  try {
    ...
    if ((_a = input.items) == null ? void 0 : _a.length) console.log("[input] items count:", input.items.length);
    const extraItems = Array.isArray(input.items) ? input.items.slice(1) : [];
    ...
    if (input.tag) {
      console.log("📍 Step 7: Adding topic/tag...");
      await page.waitForSelector("input.xwhw2v2", { timeout: 1e4 });
      await humanDelay(1e3, 2e3);
      await page.click("input.xwhw2v2");
      await humanDelay(500, 800);
      await page.keyboard.type(input.tag, { delay: 50 });
      await humanDelay(800, 1200);
      console.log("✅ Step 7 completed: Topic added");
    }
    ...
    if (extraItems.length > 0) {
      console.log(`📦 Processing ${extraItems.length} additional item(s) for this profile...`);
      for (const [idx, item] of extraItems.entries()) {
        try {
          console.log(`📍 Extra #${idx + 1}: Opening post composer...`);
          await page.waitForSelector(".x1i10hfl > .xc26acl", { timeout: 1e4 });
          ...
          if (item.postText) {
            ...
            console.log("⌨️ Extra typing post text:", item.postText);
          ...
          if (item.mediaPath) {
            ...
            await humanDelay(500, 1e3);
            await page.click(".x1uvtmcs > .__fb-dark-mode");
            await humanDelay(600, 1e3);
            const fileInput = await page.$('input[type="file"]');
            if (fileInput) {
              console.log("📷 Extra uploading image:", item.mediaPath);
              await fileInput.uploadFile(item.mediaPath);
              await humanDelay(1500, 2500);
            }
          }
          if (item.tag) {
          ...
          await page.click(".x2lah0s:nth-child(1) > .x1i10hfl > .xc26acl");
          await humanDelay(2e3, 3e3);
          console.log(`✅ Extra #${idx + 1}: Content posted`);
        } catch (e) {
          console.error(`❌ Extra #${idx + 1} failed:`, e);
        }
      }
    }
